    ...
    <div className='Sidebar fixed z-50 right-0 top-0'>
      ...
      >
        ...
        <div className='p-5 flex flex-row gap-2'>
          ...
          <div>
            <p className='text-green-600 font-bold font-inter text-lg'>DEJAY</p>
        ...
        <ul className='flex flex-col w-full px-2 gap-2 mt-2'>
          {SiteLinks.map((link) => {
            ...
            return(
              ...
              <Link href={link.link} onClick={() => setTimeout(() => {setOpen(false)}, 1100)}>
              <motion.div 
              whileTap={{scale: 0.9}}
              transition={{duration: 0.5}}
              className={`relative flex flex-row items-center gap-3 py-3 px-4 ${isActive ? 'bg-gray-100 text-green-600' : ''} rounded-md`}>
              <link.icon className='size-4'/>              
              <p className='font-inter text-md'>{link.name}</p>
              {
                isActive && (
                  <motion.div layoutId='move' className='absolute right-2 top-2 w-2 h-2 bg-green-600 rounded-full'></motion.div>
                )
              }
              </motion.div>
              </Link>          
        ...
        <div className='absolute bottom-0 w-full p-5 py-2'>
          <p className='font-inter text-xs'>&copy; Veagy Co-operation {date}</p>
        </div>
        </div>
      </Drawer>
    </div>
